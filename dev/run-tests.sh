...
if [[ "$BROWSER_ONLY" == true ]]; then
    ...
    if [[ -n "$HA_URL" ]]; then
        ...
        if [[ "$REVIEW_ONLY" == true ]]; then
            PLAYWRIGHT_ARGS="--project=desktop --grep='(router-health|network-devices|network-topology|signal-heatmap|roaming-activity|interface-health|dashboard loads|no console errors)'"
