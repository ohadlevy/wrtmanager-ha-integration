...
class WrtManagerCoordinator(DataUpdateCoordinator):
    ...
    async def _async_update_data(self) -> Dict[str, Any]:
        ...
        successful_auths = 0
        for i, (host, result) in enumerate(zip(eligible_hosts, auth_results)):
    ...
    def _handle_router_failure(self, host: str, error: Exception, operation: str) -> None:
        ...
        if self._consecutive_failures[host] <= 3:
            _LOGGER.warning(
                "Router %s failed during %s (attempt %d): %s",
                host,
                operation,
                self._consecutive_failures[host],
                error,
            )
        else:
            _LOGGER.error(
        ...
        if self._consecutive_failures[host] >= self._max_consecutive_failures:
    ...
    def get_router_status(self) -> Dict[str, Any]:
        """Get detailed status of all routers for diagnostics."""
        current_time = datetime.now()
        router_status = {}
